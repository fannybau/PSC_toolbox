...
for device in jv_fws: 
    if device.metadata["sample"] == 'main':
        ...
        if state == 'after':
            if 'NC' in name:
                ax2.plot(volt_fw, mA_fw, '--', color=col, alpha = 0.4)
            else:
                ax1.plot(volt_fw, mA_fw, '--', color=col, alpha = 0.4)
        elif state == 'before':
            ax1.plot(volt_fw, mA_fw, '--', color=col, alpha = 0.3)
            ax2.plot(volt_fw, mA_fw, '--', color=col, alpha = 0.3)
...
for device in jv_rvs: 
    if device.metadata["sample"] == 'main':
        ...
        if state == 'after':
            if 'NC' in name:
                ax2.plot(volt_rv, mA_rv, '-', color=col, alpha = 0.8, label=name+state)
            else:
                ax1.plot(volt_rv, mA_rv, '-', color=col, alpha = 0.8, label=name+state)
        elif state == 'before':
            ax1.plot(volt_rv, mA_rv, '-', color=col, alpha = 0.5)
            ax2.plot(volt_rv, mA_rv, '-', color=col, alpha = 0.5)


ax1.legend(loc='lower left')
ax2.legend(loc='lower left')
fig_path = db.add_asset('IVcompC-BeforeEIS.png')
fig = fig.savefig(fig_path)
fig_path2 = db.add_asset('IVcompNC-BeforeEIS.png')
