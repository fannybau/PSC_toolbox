...
dark_fw = list(filter(lambda asset: asset.file.endswith("8-jv-dark-before-eis-fw_2d.txt"), assets))
dark_rv = list(filter(lambda asset: asset.file.endswith("9-jv-dark-before-eis-rv_2d.txt"), assets))

for device in dark_fw: 
...
for device in dark_rv: 
    if device.metadata["sample"] == 'main':
        ...
        #volt_af, mA_af = iV_paios(after_fw.file,areas[0]) 
        #ax2.plot(volt_af, mA_af, '--', color = col, alpha = 0.6, label=name)
        #volt_ar, mA_ar = iV_paios(after_rv.file,areas[0]) 
        #ax2.plot(volt_ar, mA_ar, '-', color = col, alpha = 0.8, label=name)

ax1.legend(loc='upper left')
ax2.legend(loc='upper left')
fig_path1 = db.add_asset('darkIV_before.png')
figbefore = figbefore.savefig(fig_path1)
fig_path2 = db.add_asset('darkIV_after.png')
